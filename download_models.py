...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, help="Specific model to download (tiny, base, small, medium, large-v3, turbo)")
    ...
    if args.all:
        ...
        if args.model in ALL_MODELS:
            ...
            print(f"Error: '{args.model}' is not a valid model name. Choose from: {', '.join(ALL_MODELS)}")
    else:
        # Default behavior: list options
        print("Available models:")
        for m in ALL_MODELS:
            print(f" - {m}")
        print("\nRun with '--all' to download everything, or '--model <name>' for a specific one.")
